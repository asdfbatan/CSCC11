...
class LogisticRegression:
    ...
    def _compute_loss_and_gradient(self, X, y, alpha_inverse=0, beta_inverse=0):
        ...
        one = np.ones([N, 1]) #1st column
        Xnew = np.append(one, X, axis=1) # N*(D+1) matrix
        W = self.parameters # k*(D+1) matrix
        sigma = softmax(Xnew@W.transpose()) # N*K matrix
        K = W.shape[0]
        
        NLL_term = 0
        for i in range(N):
            NLL_term -= np.log(sigma[i, y[i]])/N
                    
        C = np.diag([alpha_inverse] * (D+1))
        C[0][0] = beta_inverse
        ...
        for k in range(K):
            wk = W[k]
            temp2 = wk.transpose()@C
            L2_term -= temp2@wk # -WtCWt
    
        nll = NLL_term + L2_term[0]
        nll = nll[0]
        ...
        p1 = Y_lab.transpose()@Xnew - sigma.transpose()@Xnew
        p2 = np.matmul(W, C)
