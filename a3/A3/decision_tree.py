...
class DecisionTree:
    ...
    def _find_split(self, X, y, H_data):
        ...
        for ii in range(unique_values.shape[0] - 1):
            ...
            # TODO: Remove this line and implement.
            current_split_index = first_idxes[ii+1]
            current_split_value = unique_values[ii]
            
            y_left = y[:current_split_index]
            y_right = y[current_split_index:]
            
            H_left = self._entropy(y_left)
            H_right = self._entropy(y_right)
            
            current_entropy = H_left * (len(y_left)/len(y)) + H_right * (len(y_right)/len(y))
            current_information_gain = H_data - current_entropy             
    ...
    def _build_tree(self, X, y, level):
        ...
        if left_split.sum() == 0 or right_split.sum() == 0:
            ...
            if self.debug:
                print(f"Num Samples: {N}, Entropy: {H_data}, Depth: {level}, Probs: {probs.T} - No split dim that gives information gain")
            return current_node
