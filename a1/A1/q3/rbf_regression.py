...
class RBFRegression():
    ...
    def predict(self, X):
        ...
        # Calculate normal dist pdf exp part
        # e^{-[(X-Ci)^2]/2sigma^2} = rbf2d
        rbf2d = []
        size = range(self.K)
        for i in size:
            rbf2d.append(self._rbf_2d(X, i))
        # one column
        array_rbf2d = np.asarray(rbf2d).T[0]
        # two column 
        array_rbf2d_2 = (array_rbf2d.shape[0], 1)

        matrix =  np.ones(array_rbf2d_2)
        
        B = np.append(matrix, array_rbf2d, axis=1)
    ...
    def fit_with_l2_regularization(self, train_X, train_Y, l2_coef):
        ...
        size = range(self.K)
        for i in size:
            rbf2d.append(self._rbf_2d(train_X, i))
        # one column
        array_rbf2d = np.asarray(rbf2d).T[0]
